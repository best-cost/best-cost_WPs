            ...
            purrr::map(
              function(.x){
                .x <- .x %>%
                  ...
                  mutate(paf_from_mid_year_pop = premature_deaths_2019 / population_2019 / (1 - prob_survival_natural)) %>% 
                  mutate(paf_from_entry_pop = premature_deaths_2019 / population_2019_entry / (1 - prob_survival_natural), .after = paf) %>% 
                  mutate(paf = 1 - modification_factor, .after = paf_from_entry_pop)
  ...
  dplyr::mutate(
    attr_ge =
      ...
      purrr::map(
        function(.x){
          .x <- .x %>% 
            # mutate(premature_deaths_2019 = population_2019_entry * (1 - prob_survival_natural) * paf) %>% 
            mutate(premature_deaths_2019 = population_2019 * (1 - prob_survival_natural) * paf) # GeLuft approach
        }
      )
  , .after = attr_air)
