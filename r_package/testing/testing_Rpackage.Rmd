...
# SHOW ACF ####
# Error: "object 'lifetable_airqplus' not found" -> duplicate code that was not deleted? cf. next code, which is almost identical
#lifetable_airqplus <- 
...
```{r Codebook}
# SHOW ACF ####
# ERROR (FIXED NOW): there doesn't seem to be a column "outcome_label_de"
# Do not forget yll
codebook <- unique(bhd_data[, c("outcome_id", "mort_morb")]) #%>%  removed "outcome_label_de" from the column selection;  
  # Removed the next lines
  # Add label for crf
...
for (r in c("2019", "whoaqg21")){ # LUYTAX: replaced "lrv" (which doesn't exist in impact_morbidities_edited) by "whoaqg21" 
  # Substract the impacts for the scenarios
  impact_morbidities_comparative[[paste0(r, "_whoaqg21")]] <- 
    dplyr::left_join(impact_morbidities_edited[[r]], # LUYTAX: ERROR (now fixed) 
...
#ERROR (before "lrv" replaced by "whoaqg21"): Error in UseMethod("left_join") : no applicable method for 'left_join' applied to an object of class "NULL"
...
impact_morbidities <- 
  ...
  # Order columns 
  dplyr::select(pollutant, term, #outcome_label_de, # removed selection of this column because german labels removed in chunk "Codebook"
                bhd,
                crf_meanCI, 
                ...
                everything()) #%>%
  # Order rows by label
  #dplyr::arrange(outcome_label_de) # removed this line because german labels removed in chunk "Codebook"
```
...
for(i in 1:nrow(input_data_mortality)){
  ...
  bestcost::calculate_mortality_usingLifetable(
...
for (r in c("2019", "whoaqg21")){ # replaced "lrv" with "whoaqg21"
  # Substract the impacts for the scenarios
...
impact_mortality <- 
  ...
  dplyr::select(pollutant, term,
                #outcome_label_de, # LUYTAX: removed
                bhd,
                crf_meanCI, 
                contains("impact_diff_meanCI"),
                contains("impact_ref_meanCI"), 
