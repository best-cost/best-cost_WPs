...
bestcost_pm_yll_lifetable_airqplus <-
  bestcost::attribute_yll_from_lifetable_airqplus(
    exp_central = airqplus_pm_deaths_yll[["input"]]$mean_concentration, 
    cutoff = airqplus_pm_deaths_yll[["input"]]$cut_off_value,
    rr_central = airqplus_pm_deaths_yll[["input"]]$relative_risk,
    rr_lower = airqplus_pm_deaths_yll[["input"]]$relative_risk_lower,
...
yll_female <- bestcost_pm_yll_lifetable_airqplus[["detailed"]][["interim"]][["lifeyears_nest"]][[2]] %>% 
  filter(year == 2019) %>% select(impact) %>% as_vector() %>%  unname(.)
print("YLL male 2019 identical:") ; round(yll_male) == round(624.43)
print("YLL female 2019 identical:") ; round(yll_female) == round(675.25)
...
airqplus_yll_difference <- read.xlsx("../testing/Replication_AirQ+/airqplus_deaths_yll_lifetable_adults.xlsx", sheet = "yll_difference", startRow = 1, rows = c(1:101)) %>% 
  select(Age, `2019`, `2020`)
yll_total <- bestcost_pm_yll_lifetable_airqplus[["detailed"]][["interim"]][["pop_impact_nest"]][[1]] %>% select(contains("population")) +
  bestcost_pm_yll_lifetable_airqplus[["detailed"]][["interim"]][["pop_impact_nest"]][[2]] %>% select(contains("population"))
yll_total <- yll_total %>% mutate(age = 0:99, .before = population_2019)
yll_total_2019_2020 <- yll_total %>% select(age, population_2019, population_2020) %>% 
  mutate(population_2019_rounded = round(population_2019), .after = age) %>% 
  mutate(population_2020_rounded = round(population_2020), .after = population_2019_rounded)

print("2019 total YLLs identical:")
table(airqplus_yll_difference$`2019` == yll_total_2019_2020$population_2019_rounded)
print("2020 total YLLs identical:")
table(airqplus_yll_difference$`2020` == yll_total_2019_2020$population_2020_rounded)
print("2020 absolute difference in YLLs:")
difference <- abs(airqplus_yll_difference$`2020` - yll_total_2019_2020$population_2020_rounded)
print(difference) # difference is always 1
# Reason for slight difference is that we compare two slightly different totals:
# - For airqplus the total YLLs are calculated from a total population lifetable
# - For bestcost, the total YLLs are calculated by summing the sex-specific YLLs, which were determined from sex-specific lifetables
# In theory they should be exactly the same, but in practice there are rounding errors (AirQ+ only provides rounded results)
```
