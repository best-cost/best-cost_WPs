...
choice_pairs_main <- 
  crf_data %>% dplyr::filter(choice_pair %in% "main") %>% select(pair_id) %>% unique() %>% pull()
choice_pairs_appendix <- 
  crf_data %>% dplyr::filter(choice_pair %in% "appendix") %>% select(pair_id) %>% unique() %>% pull()

choice_crf_all <- c(choice_pairs_main, choice_pairs_appendix)
