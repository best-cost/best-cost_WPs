...
```{r single exposure value with multiple uncertainties, eval=FALSE, include=FALSE}
...
check_general(
  a = bestcost_pm_death_social_decile[["detailed"]][["social"]]$difference_value,
  # Results on 21 Nov 2024
  b = c( 203.54994443, 0.25284914, 142.64936990, 0.15052568,
         0.65578653, 0.05604937, 0.22984830, 0.01926640, 
         22.52416423, 0.32236823, 14.5680866,0.17252793,
         0.004598529, 0.053420096, 0.001606388, 0.018319205))



bestcost_pm_copd_geo_social_multiplicative <- 
  healthiar::include_social(output = bestcost_pm_death,
                            geo_id_raw = social_data$CS01012020,
                            deprivation_score = social_data$score,
                            population = social_data$POPULATION,
...
bestcost_pm_yld_singlebhd  <- 
  healthiar::attribute_yld_prevalence_based_approach(
    exp_central = 8.85, 
    cutoff_central = 5,    
    bhd_central = 25000,
    rr_central = 1.118,
...
bestcost_pm_yld_singlebhd_incidence_based  <- 
  healthiar::attribute_yld_incidence_based_approach(
    exp_central = 8.85, 
    cutoff_central = 5,    
    bhd_central = 25000,
    rr_central = 1.118,
...
# View(bestcost_pm_yld_singlebhd_incidence_based)

# No study to compare bestcost results
# Check only if results stay the same overtime despite developments
check_bestcost(result_list_bestcost = bestcost_pm_yld_singlebhd_incidence_based ,
               result_vector_alternative = c(525*10, 277*10, 768*10)) # Result on 16 May 2024
```
...
bestcost_pm_copd_with_summary_uncertainty <- 
  healthiar::attribute_health(
    ...
    erf_increment = 10, 
    erf_shape = "log_linear", 
    summary_uncertainty = TRUE
    )

# View(bestcost_pm_copd_with_summary_uncertainty)

bestcost_pm_copd_extern_uncertainty <-
  healthiar::include_summary_uncertainty(bestcost_pm_copd_with_summary_uncertainty)

# Comparison central +- 95% CIs & Monte-Carlo central, lower, upper estimates
## Summary uncertainty from internal function (initial implementation)
print("Summary uncertainty estimates (intern calculation): ")
bestcost_pm_copd_with_summary_uncertainty[["detailed"]][["uncertainty"]]$central_estimate
bestcost_pm_copd_with_summary_uncertainty[["detailed"]][["uncertainty"]]$lower_estimate
bestcost_pm_copd_with_summary_uncertainty[["detailed"]][["uncertainty"]]$upper_estimate
print("Main estimates: ") ; bestcost_pm_copd_with_summary_uncertainty[["main"]][["impact_rounded"]]
...
bestcost_pm_copd_extern_uncertainty[["detailed"]][["uncertainty"]]$central_estimate
bestcost_pm_copd_extern_uncertainty[["detailed"]][["uncertainty"]]$lower_estimate
bestcost_pm_copd_extern_uncertainty[["detailed"]][["uncertainty"]]$upper_estimate
...
bestcost_pm_copd_geo_short_uncertainty <- 
  healthiar::include_summary_uncertainty(bestcost_pm_copd_geo_short)

# Comparison central +- 95% CIs & Monte-Carlo central, lower, upper estimates
print("Main estimate: ") ; bestcost_pm_copd_geo_short[["main"]][["impact_rounded"]]
print("Summary uncertainty estimates (extern uncertainty function): ")
round(as.numeric(bestcost_pm_copd_geo_short_uncertainty[["detailed"]][["uncertainty"]]), digits = 0)
```
...
bestcost_noise_ihd_expDist_extern_uncertainty <- include_summary_uncertainty(bestcost_noise_ihd_expDist)

# Comparison central +- 95% CIs & Monte-Carlo central, lower, upper estimates
print("Main estimate: ") ; bestcost_noise_ihd_expDist[["main"]][["impact_rounded"]]
print("Summary uncertainty estimates (extern uncertainty function): ")
bestcost_noise_ihd_expDist_extern_uncertainty[["detailed"]][["uncertainty"]]$central_estimate
bestcost_noise_ihd_expDist_extern_uncertainty[["detailed"]][["uncertainty"]]$lower_estimate
bestcost_noise_ihd_expDist_extern_uncertainty[["detailed"]][["uncertainty"]]$upper_estimate
```



```{r Summary uncertainty rr exposure distribution iteration, eval=FALSE, include=FALSE}
# Data prep
niph_noise_ihd_input <- 
  niph_noise_ihd_excel |>
  dplyr::filter(!is.na(niph_noise_ihd_excel$exposure_mean))
...
# CONTINUE HERE: EXPOSURE CATEGORIES NOT CORRECTLY INCLUDED IN OUTPUT ####
# INSTEAD OF 5 EXPOSURE CATEGORIES, THERE ARE 15 PER ROW INCLUDED (ALL EXPOSURE CATEGORIES OF THE 3 GEO UNITS) IN THE COLUMN EXP (SEE BELOW) 
View(bestcost_noise_ihd_expDist_iteration[["detailed"]][["raw"]])
bestcost_noise_ihd_expDist_iteration[["detailed"]][["raw"]]$exp[1]

# NOT WORKING YET #### 
bestcost_noise_ihd_expDist_iteration_uncertainty <- include_summary_uncertainty(bestcost_noise_ihd_expDist_iteration)

print("Main estimate: ") ; bestcost_noise_ihd_expDist_iteration[["main"]][["impact_rounded"]]
print("Summary uncertainty estimates (extern uncertainty function): ")
round(as.numeric(bestcost_noise_ihd_expDist_iteration_uncertainty[["detailed"]][["uncertainty"]]), digits = 0)
```



```{r Summary uncertainty rr attribute_yld_prevalence_based_approach, eval=FALSE, include=FALSE}
...
bestcost_pm_yld_singlebhd_external_summary_uncertainty <- healthiar::include_summary_uncertainty(bestcost_pm_yld_singlebhd_with_summary_uncertainty)

# Comparison central +- 95% CIs & Monte-Carlo central, lower, upper estimates
# View(bestcost_pm_yld_singlebhd_with_summary_uncertainty)
print("Main estimates: ") ; bestcost_pm_yld_singlebhd_with_summary_uncertainty[["main"]][["impact_rounded"]]
print("Summary uncertainty estimates (internal function): ")
round(as.numeric(bestcost_pm_yld_singlebhd_with_summary_uncertainty[["detailed"]][["uncertainty"]]), digits = 0)

# Check: probably dw's not considered correctly ####
print("Summary uncertainty estimates (external function): ")
round(as.numeric(bestcost_pm_yld_singlebhd_external_summary_uncertainty[["detailed"]][["uncertainty"]]), digits = 0)
```

```{r Summary uncertainty ar, eval=FALSE, include=FALSE}
...
# View(bestcost_noise_ha_ar_with_summary_uncertainty$detailed$uncertainty)
print("Summary uncertainty central estimate (internal function): ")
bestcost_noise_ha_ar_with_summary_uncertainty[["detailed"]][["uncertainty"]]$central_estimate
print("Main estimate: ")
bestcost_noise_ha_ar_with_summary_uncertainty[["main"]][["impact_rounded"]]

# External include_summary_uncertainty function
# Check: likely error in compilation of uncertainty at end of script ####
bestcost_noise_ha_ar_external_uncertainty <- healthiar::include_summary_uncertainty(bestcost_noise_ha_ar_with_summary_uncertainty)

# Comparison central +- 95% CIs & Monte-Carlo central, lower, upper estimates
print("Summary uncertainty estimates (extern uncertainty function): ")
bestcost_noise_ha_ar_external_uncertainty[["detailed"]][["uncertainty"]]$central_estimate
bestcost_noise_ha_ar_external_uncertainty[["detailed"]][["uncertainty"]]$lower_estimate
bestcost_noise_ha_ar_external_uncertainty[["detailed"]][["uncertainty"]]$upper_estimate

```

```{r Summary uncertainty lifetable premature deaths}

# Takes a couple of seconds to run
bestcost_pm_deaths_lifetable_airqplus_with_summary_uncertainty <-
...
# View(bestcost_pm_deaths_lifetable_airqplus_with_summary_uncertainty)

# Comparison central +- 95% CIs & Monte-Carlo central, lower, upper estimates
print("Summary uncertainty central estimate: ")
uncertainty <- bestcost_pm_deaths_lifetable_airqplus_with_summary_uncertainty[["detailed"]][["uncertainty"]]
uncertainty[uncertainty$sex == "total", "central_estimate"] |>  pull()
uncertainty[uncertainty$sex == "total", "lower_estimate"] |>  pull()
uncertainty[uncertainty$sex == "total", "upper_estimate"] |>  pull()
print("Main estimate: ")
bestcost_pm_deaths_lifetable_airqplus_with_summary_uncertainty[["main"]][["impact_rounded"]] 
```
...
```{r comparison yld singlebhd_rr prevalence-based approach}
# DELTA
# Test delta comparison function with fake values
comparison_yld_singlebhd_rr_delta <-
  healthiar::compare_yld_prevalence_based_approach(
    approach_comparison = "delta",
    exp_central_1 = 8.85, 
    exp_central_2 = 6,
    cutoff_central = 5,    
...
```{r comparison yld singlebhd_rr incidence-based approach}
# DELTA
# Test delta comparison function with fake values
comparison_yld_singlebhd_rr_delta <-
  healthiar::compare_yld_incidence_based_approach(
    approach_comparison = "delta",
    exp_central_1 = 8.85, 
    exp_central_2 = 6,
    cutoff_central = 5,    
...
# Check only if results stay the same overtime despite developments
check_bestcost(result_list_bestcost = comparison_yld_singlebhd_rr_delta,
               result_vector_alternative = c(387*10, 205*10, 564*10)) # Result on 16 May 2024
...
# Check only if results stay the same overtime despite developments
check_bestcost(result_list_bestcost = comparison_yld_singlebhd_rr_delta_geo ,
               result_vector_alternative = c(591*10, 313*10, 861*10)) # Result on 26 June 2024
...
# Check only if results stay the same overtime despite developments
check_bestcost(result_list_bestcost = comparison_yld_singlebhd_rr_pif ,
               result_vector_alternative = c(391*10,206*10,573*10)) # Result on 16 May 2024
...
# Check only if results stay the same overtime despite developments
check_bestcost(result_list_bestcost = comparison_yld_singlebhd_rr_pif_geo ,
               result_vector_alternative = c(599*10, 315*10, 878*10)) # Result on 20 June 2024
```
...
round(comparison_deaths_lifetable_delta_geo[["detailed"]][["scenario_1"]][["detailed"]][["raw"]] |>  
        dplyr::filter(geo_id_raw %in% "a") |> 
        select(impact)  |> 
        pull(), digits = 0) == 
  round(bestcost_pm_deaths_lifetable_airqplus[["main"]]$impact)
# Absolute differences
abs(comparison_deaths_lifetable_delta_geo[["detailed"]][["scenario_1"]][["detailed"]][["raw"]] |>  
