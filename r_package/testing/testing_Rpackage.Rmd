...
if(!"healthiar" %in% installed.packages()[, "Package"]){
  ...
  devtools::install_github(repo = "SwissTPH/eeh_r_packages/healthiar" ,
                           force =TRUE)
}
...
# Values in the confidence interval (ci)
...
paste_CI <- 
  function(mean, low, high, decimals){
     paste0(sprintf(paste0("%.", decimals, "f"), round(mean, digits = decimals))," [",  
            sprintf(paste0("%.", decimals, "f"), round(low, digits = decimals)), "; ", 
            sprintf(paste0("%.", decimals, "f"), round(high, digits = decimals)), "]")
  }
...
generic_data <- 
  read.csv("input/generic_data.csv",
           sep = ";")
...
population_byAge_rawdata <-
  read.csv2("input/population/population_byAge_bySex_CH_2018_2019.csv")%>%
  # Rename columns
...
for(s in sex){
  ...
  lifetable[[s]] <- 
    read.csv2(
      ...
      paste0("input/life_tables/life_table_2013_2020_",
             s,
             ".csv"))%>%
    # Rename columns
...
nonNatural_mortality_rawdata<- 
   list(
     male = read.csv2("input/mortality/nonNatural_mortality_byAge_2009_2019_male.csv",
                      na.strings =".", 
                      check.names = FALSE),
     female = read.csv2("input/mortality/nonNatural_mortality_byAge_2009_2019_female.csv",
                        na.strings =".", 
                      check.names = FALSE))
...
total_mortality_rawdata <- 
  list(
    male = read.csv2("input/mortality/total_mortality_byAge_2009_2019_male.csv",
                     na.strings =".", 
                      check.names = FALSE),
    female = read.csv2("input/mortality/total_mortality_byAge_2009_2019_female.csv",
                       na.strings =".", 
                      check.names = FALSE))
...
impact_morbidities <- 
  ...
  # Paste the CIs
  dplyr::mutate(
    crf_meanCI = paste_CI(crf_mean, crf_lowci, crf_highci, 3),
    impact_ref_meanCI = paste_CI(impact_ref_mean, impact_ref_lowci, impact_ref_highci, 3),
    impact_whoaqg21_meanCI = paste_CI(impact_whoaqg21_mean, impact_whoaqg21_lowci, impact_whoaqg21_highci, 0),
    impact_diff_meanCI = paste_CI(impact_diff_mean, impact_diff_lowci, impact_diff_highci, 0))%>%
  ...
  # Split (convert to list) by mean/appendix, selected_sensitivity 
  #%>%(., .$geluft_pair)%>%
  #purrr::map(function(x) split(x, x$pollutant))%>%
  #purrr::map(map, function(x) split(x, x$geluft_crf)) 
