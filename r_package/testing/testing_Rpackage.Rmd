...
## test using pollution and geo data for Brussels
bxl_pm25 <- terra::rast("./data/prepare/pm25_brussels_region_2020.tif") # saving the grid to the .RData file and then loading it afterwards does not work for some reason
exposure <- prepare_exposure_data(
    poll_grid = bxl_pm25,
    geo_units = bxl_sect_pop[, c('geometry')],
    population = sf::st_drop_geometry(bxl_sect_pop$population),
    geo_id_aggregated = sf::st_drop_geometry(bxl_sect_pop$mun_id)
)
```
