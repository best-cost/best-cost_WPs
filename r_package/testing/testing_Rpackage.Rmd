...
Suggestions
- Put all lines where data is loaded from excels together in one chunk at the start of the script
- When variables are not used anymore in the script, remove them with the rm(variable_name) command
...
# LUYTAX: IDEA ####
# Idea: possibly combine these functions and save in {bestcost} in "R" folder. This might also apply for other functions defined in this script?
...
percentage_pm2.5_in_pm10 <- 
  read.csv("r_package/testing/input/generic_data.csv", sep = ";") %>% 
...
```{r Editing crf (rescale inclusive)}
crf_data <- 
  crf_rawdata%>%
...
```{r Storing relevant values from crf_data}
...
```{r subsets}
# Keep only morbidity (mortality is in life tables)
bhd_data_morbidity <- bhd_data %>%
  # Keep only those rows that are relevant 
...
```{r Life table}
#TODO: AirQ+ require the death rate by age to derive probability of survings
...
```{r Create function to get lifetable with population}
...
```{r Apply function to get lifetable with population}
...
# LUYTAX: ERROR ####
# Error: "object 'lifetable_airqplus' not found" -> duplicate code that was not deleted? cfr. next code, which is almost identical
#lifetable_airqplus <- 
#  dplyr::left_join(lifetable_airqplus, 
#                   natural_mortality,
#                   by = "age")
...
exp_data <- 
  ...
  # Filter keeping only relevant rows 
...
```{r Codebook}
# LUYTAX: ERROR ####
# ERROR: there doesn't seem to be a column "outcome_label_de". Should "outcome_label_de" be removed from the statement?
# Do not forget yll
codebook <- unique(bhd_data[, c("outcome_id", "outcome_label_de", "mort_morb")]) %>%  "outcome_label_de";  
  # Add label for crf
  ...
  mutate(crf_label_de = ifelse(outcome_id %in% c("premature deaths_all causes_adults", "yll_all causes_adults"),
                               ...
                               ifelse(outcome_id %in% c("premature deaths_all causes_infants",
                                                        "yll_all causes_infants"),
...
for(i in 1:nrow(input_data_morbidities)){
  ...
  bestcost::calculate_healthImpacts(
...
for (r in c("2019", "lrv")){
  ...
  impact_morbidities_comparative[[paste0(r, "_whoaqg21")]] <- 
    dplyr::left_join(impact_morbidities_edited[[r]], # LUYTAX: ERROR #### 
...
#ERROR: Error in UseMethod("left_join") : no applicable method for 'left_join' applied to an object of class "NULL"


# Next code refers to dataframe "codebook" -> it doesn't run because of the earlier error in chunk 23 "Codebook" 
impact_morbidities <- 
  ...
  dplyr::left_join(., codebook, # LUYTAX: ERROR ####
                   by = "outcome_id",
                   relationship = "many-to-one")%>%
...
# Calculate using bestcost

# ERROR: Error in loadNamespace(x) : there is no package called ‘healthiar’
# checked .libPaths()
# [1] "C:/Users/luytax/Documents/R/win-library/4.1" "C:/Program Files/R/library"
# In second path there is no folder "bestcost" ; admin right needed to edit the folder. But I'm not sure that's the reason for the error.
for(i in 1:nrow(input_data_mortality)){
  ...
  bestcost:::calculate_mortality_usingLifetable( # LUYTAX: ERROR ####
...
# Error in `dplyr::select()`: Column `impact_per_unit` doesn't exist.
impact_mortality <- 
  impact_mortality_comparative %>%
  dplyr::bind_rows(.) %>%
  # Remove non relevant columns
  dplyr::select(-impact_per_unit, -case) %>% # LUYTAX: ERROR
...
```{r}
# LUYTAX: QUESTION
# This code seems like it belongs in the chunk "Lifetable as in Airq+" ?
write.csv2(lifetable_airqplus,
            "r_package/testing/output/lifetable_airqplus_2019_ch.csv", 
            row.names = FALSE)
```