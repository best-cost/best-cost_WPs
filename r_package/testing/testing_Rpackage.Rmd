...
crf_data <- 
  ...
  # Reshape CRFs from wide to long 
  # to enable editing CRFs by row
...
# Pollutant-outocome pairs to be considered in the main analysis or in the appendix
choice_pairs_main <- 
  crf_data %>% dplyr::filter(choice_pair %in% "main") %>% 
  select(pair_id) %>% unique() %>% pull()
choice_pairs_appendix <- 
  crf_data %>% dplyr::filter(choice_pair %in% "appendix") %>%
  select(pair_id) %>% unique() %>% pull()
...
lifetable_airqplus <- 
  ...
  dplyr::mutate(age_end = age, .after = age)f
