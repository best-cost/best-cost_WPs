...
check_bestcost(result_list_bestcost = bestcost_pm_deaths_lifetable_airqplus_copy,
               result_vector_alternative = c(2600.9, 1371, 3804))
```
```{r Speed test premature deaths}
system.time(replicate(100, bestcost::attribute(
...
system.time(replicate(100, bestcost::attribute_copy(
    health_metric = "death_from_lifetable",
    exp_central = airqplus_pm_deaths_yll[["input"]]$mean_concentration, 
    cutoff = airqplus_pm_deaths_yll[["input"]]$cut_off_value,
    rr_central = airqplus_pm_deaths_yll[["input"]]$relative_risk,
...
# Calculate years lived with disability using new bestcost function
# with input data from the Swiss project GeLuft

bestcost_pm_yld_lifetable_geluft_new  <- 
  attribute_copy(
...
# But disability weight = 1 in the function above, then YLD = YLL 
# and the result should be identical.
check_bestcost(result_list_bestcost = bestcost_pm_yld_lifetable_geluft_new,
               result_vector_alternative = c(27742, 14641, 40542))
```
