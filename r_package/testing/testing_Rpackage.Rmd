...
```{r Alternative 1 to load bestcost: load zip from local source, eval=FALSE, include=FALSE}
...
att_morb_bestcost <-
  bestcost::attribute_health_singlebhd_rr(
    exp = airqplus_morb$mean_concentration,
    cutoff = airqplus_morb$cut_off_value,    # PM2.5=5, NO2=10, i.e. WHO AQG 2021 
    bhd = airqplus_morb$incidents_per_100_000_per_year/1E5*airqplus_morb$population_at_risk,
    rr = c(airqplus_morb$relative_risk, airqplus_morb$relative_risk_lower, airqplus_morb$relative_risk_upper),
    rr_increment = 10, 
    erf_shape = "log_linear", 
    info = paste0(airqplus_morb$pollutant,"_", airqplus_morb$evaluation_name))

# Store the central estimate of the bestcost assessment
result_att_morb_bestcost <- 
  att_morb_bestcost[["total"]]%>%
  # Set order of rows
  arrange(factor(rr_ci, levels = c("mean", "low", "high")))%>%
  dplyr::select(impact_rounded) %>%
  dplyr::pull()

# Store the central estimate of the AirQ+ assessment
result_att_morb_airqplus <- 
  airqplus_morb %>%
...
result_att_morb_bestcost - result_att_mob_airqplus 
if(all((result_att_morb_bestcost - result_att_mob_airqplus) %in% 0)){
  cat(crayon::blue("Right. Bestcost and alternative source with identical results"))
} else{
  cat(crayon::red("Attention! Check! Bestcost and alternative source with different results"))}
       


# Check difference between bestcost and AirQ+ results
...
for(i in 12){ # runs loop only for health outcome "asthma incidence 20+"
...
test_1bhd_1exp <-
  bestcost::attribute_health_singlebhd_rr(
    #dw = 0.133, # for health state "Asthma, uncontrolled" (cf. bestcost::disability_weights)
    exp = airqplus_input_morb$value[airqplus_input_morb$section_name %in% "evaluation_parameters" &
                                      airqplus_input_morb$variable %in% concentration],  # PM2.5=8.30=limit LRV CH, NO2=16.32=mean conc. in CH in 2019
    cutoff = input_data_morbidities$cutoff[i],    # PM2.5=5, NO2=10, i.e. WHO AQG 2021 
    bhd = input_data_morbidities$bhd_absolute[i],
    rr = unlist(input_data_morbidities[i, 
                            c("rr_mean", "rr_lowci", "rr_highci")]),
