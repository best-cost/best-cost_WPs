...
# Create function to join mean RR with the lower and upper bound
paste_CI <- 
...
check_bestcost <-
  ...
  cat(paste("Difference (central, lower, upper): ",
            paste(result_vector_bestcost - result_vector_alternative, collapse = "; "),
            "\n"))
  # Second show message with colors
  if(all((result_vector_bestcost - result_vector_alternative ) %in% 0)){
    cat(paste(crayon::cyan("Right. Bestcost and alternative source with identical results.")))
  } else{
    cat(paste(crayon::red("Attention! Check! Bestcost and alternative source with different results.")))}
  
  }



digest_airqplus_export <- 
...
cutoff_rawdata <- 
  # Read data
  read.csv("../testing/input/cutoff.csv",
           sep = ";",
           na.strings=c("NA", ""))
...
```{r Digest input data from AirQ+}
airqplus_morb_export <-
  # Read csv file
  readr::read_delim("../testing/airqplus_cases/Impact Assessment - Switzerland (PM2.5)_ALRI_2019.csv",
                    delim = ";",
                    col_types = cols())
# Digest the data (make it ready for testing the package)
airqplus_morb <- 
  digest_airqplus_export(airqplus_export = airqplus_morb_export)
...
rr_data <- 
  ...
  tidyr::pivot_longer(c(rr_mean, rr_lowci, rr_highci),
                      names_to = "ci",
                      names_prefix = "rr_", 
                      values_to = "rr") %>%
...
cutoff_data <-
  ...
  dplyr::select(pollutant, cutoff_mean, cutoff_unit) %>%
  # Rename cutoff_mean to cutoff 
  # TODO: to be changed if we used cutoff with CI
  dplyr::rename(cutoff = cutoff_mean)
...
save.image(file = "../testing/input/input_data_for_testing_Rpackage.RData")
```
