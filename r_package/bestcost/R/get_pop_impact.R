...
get_pop_impact <-
  function(lifetable_with_pop,
    ...
    #
    #     # Determine year i+1 mid-year population using year i mid-year population (test): Results differ slightly
    #     # pop_modelled_male[(i + 1):length_period, paste0("population_test_", years[i] + 1)] <-
    #     #   pop_modelled_male[i:(length_period - 1), paste0("population_", years[i])] * pop_modelled_male[i:(length_period - 1), "prob_survival_until_mid_year"] * pop_modelled_male[(i + 1):length_period, "prob_survival_until_mid_year"]
    #
    #   }
    #
    ...
    #     mutate(!!paste0("population_",year_of_analysis,"_entry_total") := !!sym(paste0("population_",year_of_analysis)) + (deaths_total / 2)) %>%
    #     # Calculate hazard rates (absolute and percent)
    #     mutate(hazard_rate_total = deaths_total * input_with_risk_and_pop_fraction$modification_factor[1] / !!sym(paste0("population_",year_of_analysis)) ) %>%
    #     mutate(hazard_rate_percent_total = hazard_rate_total * 100) %>%
